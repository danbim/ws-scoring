...
describe("calculateJumpTotal", () => {
  ...
  it("should only count one jump per type, then take top N", () => {
    // Rider has:
    // - Two forward: 7.5 and 6.5 points
    // - One backloop: 5 points
    // - One tableTop: 4 points
    const scores: JumpScore[] = [
      ...
      createJumpScore("rider-1", 4, "tableTop", "score-4"),
    ];

    // With jumpsCounting=2, should get:
    // Best per type: forward=7.5, backloop=5, tableTop=4
    // Top 2: 7.5 + 5 = 12.5
    const result = calculateJumpTotal("rider-1", "test-judge", scores, 2);
  ...
  it("should handle single jump per type correctly", () => {
    const scores: JumpScore[] = [
      ...
      createJumpScore("rider-1", 4.0, "pushLoop", "score-3"),
    ];
  ...
  it("should handle jumpsCounting greater than available jump types", () => {
    ...
    const result = calculateJumpTotal("rider-1", "test-judge", scores, 5);
    expect(result).toBe(14.0);
  });
  ...
  it("should handle all jump types correctly", () => {
    ...
    const result = calculateJumpTotal("rider-1", "test-judge", scores, 3);
